...
struct player player1 = {0, 0, 0, 0, -1, 0x2C3B}; //start top left
struct player player2 = {SCREEN_WIDTH - 2*BLOCK_SIDELENGTH, SCREEN_HEIGHT - BLOCK_SIDELENGTH, 0, 0, -1, 0xF1C8}; //start bottom right
...
void start_game(){
	...
	restart = false;
	paused = false;
	player1 = (struct player) {0, 0, 0, 0, -1, 0x2C3B};
	player2 = (struct player) {SCREEN_WIDTH - 2*BLOCK_SIDELENGTH, SCREEN_HEIGHT - BLOCK_SIDELENGTH, 0, 0, -1, 0xF1C8};
}
...
void increment(struct player *p){
	...
	if(direction == 1){ //right
		...
		dx = -1*BLOCK_SIDELENGTH;
		dy = 0;
	}else if(direction == 4){ //up
		dx = 0;
		dy = -1*BLOCK_SIDELENGTH;
	}
...
void draw(){
	draw_rectangle(player1.x_coord, BLOCK_SIDELENGTH, player1.y_coord, BLOCK_SIDELENGTH, player1.col);
	draw_rectangle(player2.x_coord, BLOCK_SIDELENGTH, player2.y_coord, BLOCK_SIDELENGTH, player2.col);
	draw_countdown();
